...
public class runJob {
	...
	static public void main(String[] args) {
		...
		try {
			...
			env.put("weblogic.security.TrustKeyStore", "CustomTrust");
			env.put("weblogic.security.CustomTrustKeyStoreFileName", truststorepath);
			env.put("weblogic.security.CustomTrustKeyStorePassPhrase", truststorepass);
			env.put("weblogic.security.SSL.ignoreHostnameVerification", "true");
			
			
			/* If above doesn't work, pls use this */
			...
			System.out.println("Connecting as " + userName + " with password : " + passwd);
		...
		System.out.println("Running Jobi to query : "+jobName);
		...
		try {
		   ...
		   System.out.println("Retrieved job from schedulerTask by querying " + jobName + " is : " + user_operations_jobs);
		   if(user_operations_jobs == null ) {
			System.out.println("We got a null response when queried...lets query all jobs");
			for (String user_job : schedulerService.getAllJobs()) {
				System.out.println("Retrieved job : " + user_job);
			}
			System.out.println("Cannot find job name : " + jobName + ". Exiting with failure...");
			System.exit(1);
		   }
