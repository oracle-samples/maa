...
## fmwadbs_switch_db_conn.sh script version 1.0.
##
## Copyright (c) 2022 Oracle and/or its affiliates
## Licensed under the Universal Permissive License v 1.0 as shown at https://oss.oracle.com/licenses/upl/
##
### This script can be executed in any WLS DOMAIN using ADBS where the datasources are using tns alias.
### It replaces the existing connect information with a new ADBS WALLET. The script checks the validity of the new wallet and password
### Notice that the change in datasources requires the WLS servers to be restarted to be effective
###
###      ./fmwadbs_switch_db_conn.sh [WALLET_DIR] [WALLET_PASSWORD]
### Where:
###	WALLET_DIR:
...
gather_ds_info() {
	...
	export old_wallet_password=$(${exec_path}/fmwadbs_get_ds_property.sh $datasource_file 'javax.net.ssl.keyStorePassword')
	export old_tns_amin=$(${exec_path}/fmwadbs_get_ds_property.sh $datasource_file 'oracle.net.tns_admin')
}
