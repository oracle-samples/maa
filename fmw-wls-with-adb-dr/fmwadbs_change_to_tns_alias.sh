...
## fmwadbs_change_to_tns_alias.sh script version 1.0.
##
...
# The service levels for a DB service in ADBS are:
# low
# mid
# high
# tp
...
### Usage:
###	./fmwadbs_change_to_tns_alias.sh alias
###
### Where:
###	alias is the alias for the entry in tnsnames.ora which will typically be based on the service_level used during provisioning
### EXAMPLE:
### 	./fmwadbs_change_to_tns_alias.sh soaadb1_low
...
gather_current_variables_from_DS() {
        ...
        #export current_connect_string=$(grep url ${datasource_file} | awk -F ':@' '{print $2}' |awk -F '</url>' '{print $1}')
	export current_connect_string=$($exec_path/fmwadbs_get_connect_string.sh $datasource_file)
        export current_jps_connect_string=$(grep url ${jps_file} | awk -F ':@' '{print $2}' |awk -F '"/>' '{print $1}')
	export tns_admin=$($exec_path/fmwadbs_get_ds_property.sh $datasource_file 'oracle.net.tns_admin')
	echo ""
}
